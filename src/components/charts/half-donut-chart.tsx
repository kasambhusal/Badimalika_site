...
export function HalfDonutChart({ data, title = "Half Donut Chart", className }: HalfDonutChartProps) {
  const [activeSegment, setActiveSegment] = useState<string | null>(null)
  const [chartId, setChartId] = useState<string>("")

  useEffect(() => {
    const id = `half-donut-chart-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
    setChartId(id)
  }, [])
                  ...
                  {data.map((entry, index) => {
                    ...
                    return (
                      <Cell
                        key={`cell-${index}`}
                        fill={baseColor}
                        opacity={activeSegment === null || activeSegment === entry.name ? 1 : 0.7}
                      ...
                      return (
                        <div className="bg-white p-2 border rounded shadow-lg">
                          ...
                          <p className="text-sm text-gray-600">
                            Percentage: {(((payload[0].value as number) / data.value) * 100).toFixed(1)}%
                          </p>
                        </div>
                      )
                    }
